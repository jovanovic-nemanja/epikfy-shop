<table cellpadding="0" cellspacing="0" style="margin-top: 20px; font-family: 'Lato', sans-serif; font-style: normal; font-weight: normal; width: 100%; background-color: #E9EAED;height: 100%;border-radius: 3px;">
    <tr>
        ...
        <td style="background-color: #065fa3; border-top-right-radius:3px; padding: 10px 0 0 10px; color: #FFF;">
            @if (isset($company))
                <a href="{{ env('MAIN_SERVER') }}" style="color: white; text-decoration: none;transition: .25s;"><h1>{{ $company['name'] }}</h1></a>
            @endif
...
<div align="center" style="font-family: sans-serif; font-style: normal; font-weight: normal; width: 100%; font-size: 12px; height: 30px; color: #999999; padding-top: 10px;">
    ...
    <div style="float: left">
        <a target="_blank" href="{{ env('MAIN_SERVER') }}about">{{ trans('company.about') }}</a>&nbsp;/&nbsp;
        <a target="_blank" href="{{ env('MAIN_SERVER') }}refunds">{{ trans('company.refunds') }}</a>&nbsp;/&nbsp;
        <a target="_blank" href="{{ env('MAIN_SERVER') }}privacy">{{ trans('company.terms') }}</a>&nbsp;/&nbsp;

        @if ($company['twitter']!='')
            &nbsp;/&nbsp;<a target="_blank" href="https://twitter.com/{{ str_replace('@','', $company['twitter']) }}">{{ trans('globals.twitter_label') }}</a>
        @endif

        @if ($company['facebook']!='')
            &nbsp;/&nbsp;<a target="_blank" href="https://www.facebook.com/{{ $company['facebook'] }}">{{ trans('globals.facebook_label') }}</a>
        @endif
